...
SELECT 
	profiles.gender AS winners, -- получаем гендер
	count(likes.id) AS likes_count
	FROM profiles
	LEFT JOIN likes 
		ON profiles.user_id = likes.user_id -- присоединяем лайки, включая в запрос всех пользователей
	GROUP BY profiles.gender -- группируем по полу
	ORDER BY likes_count DESC -- сортируем по результатам от максимального
	LIMIT 1 -- и оставляем одну строку.
;
